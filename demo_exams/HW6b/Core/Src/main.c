...
uint16_t address = 0x28 << 1; // 8 bit address
...
const uint8_t SCALE = 2; // +-2G
const float SENSITIVITY = SCALE * 2.f / MAX_SAMPLING_VALUE;
...
int main(void) {
	...
	if (data != 0x33) {
		sprintf(printBuffer, "Error validating the WHO AM I. Answer: %X\r\n",
				data);
		HAL_UART_Transmit(&huart2, (uint8_t*) printBuffer, strlen(printBuffer),
				HAL_MAX_DELAY);
		return 1;
	}
...
/* USER CODE BEGIN 4 */
//We request 5 byte to account of reserved registers
uint8_t i2cData[5];
...
void HAL_I2C_MasterRxCpltCallback(I2C_HandleTypeDef *hi2c) {
	...
	sprintf(printBuffer, "X: %.2f g\n\rY: %.2f g\n\rZ: %.2f g\r\n",
			((int8_t) i2cData[0]) * SENSITIVITY,
			((int8_t) i2cData[2]) * SENSITIVITY,
			((int8_t) i2cData[4]) * SENSITIVITY);

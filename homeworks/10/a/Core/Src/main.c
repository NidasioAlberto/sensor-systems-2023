...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if (htim == &htim3) {
		if (ir_ctrl.current_byte >= 0
				&& ir_ctrl.current_byte < ir_ctrl.buffer_length) {
			...
			if (ir_ctrl.next_bit == -1) {
				...
				// Send a bit
				if (ir_ctrl.buffer[ir_ctrl.current_byte] >> ir_ctrl.next_bit
						& 0x1) {
...
void sendStringIR(uint8_t *data, int length) {
	ir_ctrl.buffer = data;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (huart == &huart1) {
		...
		// Send what we read from the terminal through the IR link
		sendStringIR(&uart2_buffer, 1);
...
int main(void) {
	...
	// Start the reception of bytes from both uarts
	HAL_UART_Receive_IT(&huart1, &uart1_buffer, 1);
	HAL_UART_Receive_IT(&huart2, &uart2_buffer, 1);
